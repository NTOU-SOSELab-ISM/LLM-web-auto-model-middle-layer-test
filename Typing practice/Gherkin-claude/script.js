...
const colors = {
    light: { correct: '#27ae60', incorrect: '#e74c3c', pending: '#7f8c8d' },
    dark: { correct: '#2ecc71', incorrect: '#e74c3c', pending: '#bdc3c7' }
};
...
function displayText() {
    elements.textDisplay.innerHTML = state.currentText.split('').map(char => 
        `<span style="color: ${colors.light.pending};">${char}</span>`
    ).join('');
    updateTextColors();
}
...
elements.closeResults.addEventListener('click', function() {
    ...
    state = { startTime: null, timerInterval: null, currentText: '', currentIndex: 0, errors: 0, totalKeystrokes: 0 };
    generateText();
});
